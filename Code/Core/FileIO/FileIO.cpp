...
{
#if defined( __WINDOWS__ )
    DWORD flags = (linkBehavior == FOLLOW_NEITHER) ? COPY_FILE_COPY_SYMLINK : 0;
    ...
    if ( linkBehavior == FOLLOW_SRC && allowOverwrite )
    {
        ...
        if ( INVALID_FILE_ATTRIBUTES != INVALID_FILE_ATTRIBUTES &&
            ( dwAttrs & FILE_ATTRIBUTE_REPARSE_POINT ) )
        {
            HANDLE hFile = CreateFile( dstFileName, 0, FILE_SHARE_VALID_FLAGS, 0,
                OPEN_EXISTING, FILE_FLAG_OPEN_REPARSE_POINT|FILE_FLAG_BACKUP_SEMANTICS, 0);
            VERIFY(hFile != INVALID_HANDLE_VALUE);
            char buf[ sizeof(REPARSE_DATA_BUFFER) + 0x100 ];
            DWORD bufSize = 0;
            bool isSymLink = false;
            if ( DeviceIoControl( hFile, FSCTL_GET_REPARSE_POINT, sizeof(buf), &bufSize, NULL ) )
            {
